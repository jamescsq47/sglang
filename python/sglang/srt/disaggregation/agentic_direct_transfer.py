...
class AgenticDirectRuntime:
    ...
    def layout_hash(self) -> str:
        ...
        fields = (
            ...
            tuple(str(value) for value in getattr(args, "state_types", ())),
            tuple(
                tuple(int(value) for value in component)
                for component in getattr(args, "state_item_lens", ())
            ),
            tuple(
                tuple(int(value) for value in component)
                for component in getattr(args, "state_dim_per_tensor", ())
            ),
        )
...
def _make_kv_args(
    *,
    ...
    # Qwen3.5 is a hybrid attention/GDN model.  Its reusable prefix is not
    # complete unless the one-slot temporal/conv state is transferred beside
    # the attention pages.  Reuse the native 0.5.14 state descriptor so the
    # reverse NIXL plane has exactly the same wire layout as normal P->D.
    setup_state_kv_args(
        kv_args,
        kv_pool,
